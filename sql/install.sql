...
-- ---------------------------------------------------------------------
-- STABLE BUSINESS — ownership + till. One row per OWNED stable (`stable_id`
-- is the config/stables.lua key). The shared backbone for BOTH management
-- menus (admin: assign owner/breeds; owner: manage employees/store) and the
-- player-run store. Sales/payouts move the till; withdraw is owner-gated. A
-- stable with no row here is unowned (admin-run, current behaviour).
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `sovereign_stable_business` (
  `stable_id`    VARCHAR(48)   NOT NULL,   -- config/stables.lua key, e.g. 'valentine'
  `owner_charid` INT(11)       NULL,       -- current owner (NULL = unowned)
  `till_cash`    DECIMAL(12,2) NOT NULL DEFAULT 0,
  `till_gold`    DECIMAL(12,2) NOT NULL DEFAULT 0,
  `created_at`   TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- ---------------------------------------------------------------------
-- STABLE EMPLOYEES — staff roster + role per stable. A `role` maps to a
-- permission set in config (trade the store, manage stock/listings, fill buy
-- orders, withdraw...). The OWNER is on the business row above, not here.
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `sovereign_stable_employees` (
  `id`         INT(11)     NOT NULL AUTO_INCREMENT,
  `stable_id`  VARCHAR(48) NOT NULL,
  `charid`     INT(11)     NOT NULL,
  `role`       VARCHAR(32) NOT NULL DEFAULT 'clerk',   -- maps to a permission set in config
...
-- ---------------------------------------------------------------------
-- STORE LISTINGS — items offered for SALE. Stock lives in the store's own
-- vorp stash ('sovereign_store_<stable_id>'); a listing prices a quantity of
-- an item drawn from that stock. Buyers pay the till and take from stock.
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `sovereign_store_listings` (
  `id`         INT(11)       NOT NULL AUTO_INCREMENT,
  `stable_id`  VARCHAR(48)   NOT NULL,
  `item`       VARCHAR(64)   NOT NULL,
  `qty`        INT(11)       NOT NULL DEFAULT 0,   -- units offered (bounded by stock)
  `price_cash` DECIMAL(12,2) NOT NULL DEFAULT 0,   -- per unit
  `price_gold` DECIMAL(12,2) NOT NULL DEFAULT 0,
  `listed_by`  INT(11)       NULL,
  `created_at` TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
-- ---------------------------------------------------------------------
-- STORE BUY ORDERS — standing offers to BUY an item from any player. Filling
-- one pays the seller from the till (per-unit) and deposits the item into the
-- store stock. `qty_filled` tracks progress toward `qty_wanted`.
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `sovereign_store_orders` (
  `id`         INT(11)       NOT NULL AUTO_INCREMENT,
  `stable_id`  VARCHAR(48)   NOT NULL,
  `item`       VARCHAR(64)   NOT NULL,
  `qty_wanted` INT(11)       NOT NULL DEFAULT 0,
  `qty_filled` INT(11)       NOT NULL DEFAULT 0,
  `price_cash` DECIMAL(12,2) NOT NULL DEFAULT 0,   -- per unit paid to the seller
  `price_gold` DECIMAL(12,2) NOT NULL DEFAULT 0,

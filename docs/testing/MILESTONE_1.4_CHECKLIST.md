...
| Q3 | **What is a wrecked wagon?** You destroyed one and the stable handed you a fresh one — obviously wrong. **Your repair ruling changed this question.** When I first asked, (b) was unviable — no repair system existed, so it would have bricked a destroyed wagon forever. Now there is one: **anyone** field-repairs to 150 and limps home; only a **Wagon Maker** reaches 100%. The harsh rule is now a real loop, not a dead end. **(a)** comes back at 150, battered but driveable. **(b)** can't leave the stable until field-repaired there. Shipping (a). **Rule:** (a) or (b)? Config: `Config.WagonDamage.wreckedNeedsRepair`. **Also open:** WHERE does a field repair happen — a kit item, a prompt at the wagon, or only at a stable? Does it cost? That decides whether being wrecked in the wilderness is an inconvenience or a story | |

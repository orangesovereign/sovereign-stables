...
**Status:** **✅ PHASE 1 CLOSED 2026-07-15 — vorp_stables matched.** *(Carried caveat: two-player transfer VII–VIII built but unverified — run in the first duo session.)* Phase 0 ✅ · Phase 1 spike gate ✅ · **Milestone 1.1 ✅ PASSED** ([ledger](https://claude.ai/code/artifact/07b8240a-3920-4d62-aa86-6fbef652450e)) — stable spine, grooming stablehand, branded storefront + live orbital preview · **Milestone 1.2 ✅ PASSED 16/16** 2026-07-14 ([ledger](https://claude.ai/code/artifact/c99403ce-30ed-4db3-b2a0-28da0d2e1038)) — server-authoritative buy→own, DB persistence, caps, anti-dupe, ledger, My Horses + default ride. · **Milestone 1.3 — summon & field ✅ PASSED** (its carried-over lines — front-spawn, walk-off dismiss, and X1 the F8 console — were folded into 1.4's Article II and verified there; spawn later re-tuned to 4m stationary per owner C1/C2) · **Milestone 1.4 — wagons · tack · transfer ✅ PASSED** ([ledger](https://claude.ai/code/artifact/c5eb610c-fd6f-411a-ab18-e05a117dc001) · [mirror](docs/testing/MILESTONE_1.4_CHECKLIST.md)) — wagons (buy/collect-at-stable/dismiss + **Condition wear & repair**, modelled on bcc-wagons since RDR3 exposes no readable wagon health — see `CREDITS.md`), **player-owned tack** (`sovereign_tack` — buy once, wear on any horse; 479 component hashes), and **transfer by session id** (the machinery the trainer's Phase 3 custody reuses). Plus the **Wagon Maker grade** + grade enforcement (grades are roles, not ranks — `tests/perms_spec.py`) and the **three-tier customization** ruling.

## ✅ PHASE 1 CLOSED — 2026-07-15 (owner decision). sovereign_stables has matched vorp_stables.

> ⚠️ **One carried caveat:** the **two-player transfer articles (VII–VIII)** — `/sovgive`, `/sovgivewagon`, and the accept/decline/distance/cap/expiry guards — are **BUILT but NOT verified in game**; they need a second player and were never run. All guards are server-side. **First duo session should run them**; if `sovereign_transfer` misbehaves, this is the untested surface. Everything else in Phase 1 is gate-passed.

**Next: Phase 2** — care/status/persistence depth + the death rework. Well-prepped: dirt/clean natives found (coal_stables), the customiser has its pipeline (`_SET_META_PED_TAG`), death rules ruled. See `03-CODING-PLAN.md`.

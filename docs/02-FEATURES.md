...
| D8 | Stumble/fall on rail tracks & dangerous terrain (toggle, trait-linked). **`ATB_RagdollEasily` (71) moves the odds — owner-confirmed.** Closes in the same pass as E3/E4; `SurfaceIncline` blackboard readable | EXCEED | ✅ | personality | 4 | ⬜ |
...
| E10 | **The repertoire** — a horse only learns the abilities it was actually *taught*. Each move must be worked N times or the horse never learns it: mirror all night and you get a level-4 horse that follows beautifully but **cannot dance, rear, or clear an obstacle**. Silent (no announcement) — **never displayed to anyone, ever** (ruled 2026-07-15; the appraisal line is struck — there is no readout to sell). The owner discovers it by riding. **Contrast E4 courage, which *is* visible to the owner** | NEW | 🧪 | E9, `training` JSON col | 3 | ⬜ |
| E2 | XP modifiers by horse size & activity (config) | EXCEED | ✅ | E1 | 3 | ⬜ |
| E3 | **Spook/fear resistance** — driven by the **animal tuning surface** (`ATF_SpookedRangeOverride` 146, `ATF_FearRange` 10, per-caliber flee ranges), state read live off the `Spooked` blackboard. *Not* a native bond ladder — that was a misread of `tutorial_flags.lua`, see [07-HORSE-TRAINER](07-HORSE-TRAINER.md) | EXCEED | ✅ | tuning natives | 3 | ⬜ |
| E4 | **Courage training — 0–9 leveled, Horse-Trainer-only (J9), foals excluded.** Expose the horse to a fear animal (gator/snake/wolf/cougar/bear); XP accrues **per second in radius**; trainer pats (on foot) or calms (mounted) to keep it there — all delivered by `EVENT_CALM_PED` (calmer · mount · `CalmTypeId` · `isFullyCalmed`). XP/sec scales with the animal's real danger. At **9**: barely spooks, tolerates close range, **won't throw the rider**. **Visible to the owner** (right-click at horse → info). **Persists for life** (DB + re-apply on spawn). See [07-HORSE-TRAINER](07-HORSE-TRAINER.md#-courage-training-e3e4--ruled-2026-07-15) | EXCEED | ✅ | E3, E5 floors, `EVENT_CALM_PED` | 3 | ⬜ |
| E5 | Personality & behavior system (traits). **⚠️ The personality→starting-tuning-param mapping must land in Phase 3** (E4 depends on it for breed/personality courage floors); the rest of the trait system stays Phase 4 | EXCEED | 🧪 | db | 3 / 4 | ⬜ |
...
2. **Bonding/courage natives** (E3, E4, **E10**, **D8**) — is RDR3's built-in horse-bonding stat readable/writable, or do we simulate our own? **Now higher priority + wider:** `rdr3_discoveries` shows the game already gates horse abilities behind a **1–4 bond ladder** (`TF_HORSE_BONDLVL_2/3/4_PERKS`, `TF_HORSE_BOND_LOCK_ACTION`) — the same 1–4 shape as our training tiers — and the owner notes these behaviours run **on chance** natively (like the track-stumble, D8). So E3 (spooking), D8 (stumble) and E10 (repertoire) may all be **one native system**. Spike must answer: can bond level be read/written, do the perk flags fire from it, and are the success chances (rear/obstacle/stumble) reachable? Decides whether E10 drives native bonding, layers its own per-move chance on top, or both.
